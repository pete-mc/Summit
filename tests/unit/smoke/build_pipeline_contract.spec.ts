...
describe('build pipeline contract', () => {
  ...
  it('keeps webpack output and copied assets aligned to extension contract', () => {
    ...
    ) as { patterns?: Array<{ from: string; to?: string }> } | undefined;
    const patterns = copyPlugin?.patterns ?? [];
    expect(patterns).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ from: 'manifest.json' }),
        expect.objectContaining({ from: 'src/summitloader.js', to: 'scripts' }),
        expect.objectContaining({ from: 'src/styles/fluent.min.css', to: 'styles' }),
      ]),
    );
  });
