...
import {
  processGuids,
  reconstructGuids,
  processGuidsAndDates,
  reconstructGuidsAndDates,
} from '@/helpers/CompressGuids';
...
describe('CompressGuids helpers', () => {
  ...
  it('roundtrips guid/date tuples through processGuidsAndDates/reconstructGuidsAndDates', () => {
    ...
    expect(reconstructed.map((item) => moment(item.date).format('DDMMYY'))).toEqual(
      guidsAndDates.map((item) => moment(item.date).format('DDMMYY')),
    );
  });
