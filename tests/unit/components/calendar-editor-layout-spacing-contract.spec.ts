...
describe("Phase 1 calendar editor layout spacing contract", () => {
  ...
  it("editor labels flow as block-level field groups instead of raw inline label content", () => {
    const css = fs.readFileSync(STYLES_PATH, "utf8");
    const source = fs.readFileSync(SUMMIT_CALENDAR_PATH, "utf8");
    const editorLabelRule = readCssRule(css, ".editor-container > label");

    expect(source).toContain('className="editor-container"');
    expect(editorLabelRule).toMatch(/display\s*:\s*(block|flex|grid)/);
    expect(editorLabelRule).toMatch(/gap\s*:\s*var\(--summit-space-(xs|sm|md)\)/);
  });
