...
import { Column, ColumnDirective, ColumnsDirective, ContentType, ExcelExport, ExcelExportProperties, ExcelQueryCellInfoEventArgs, GridComponent, Inject, PageOrientation, PdfExport, PdfExportProperties, PdfHeader, PdfPageSize, PdfQueryCellInfoEventArgs, Sort, Toolbar, ValueType } from '@syncfusion/ej2-react-grids'
...
export class OasReportTable extends React.Component<OasReportTableProps> {
  ...
  oasTemplate = ((props: OasReportItem, column: string) => {
    ...
    return props[column].map((stream: OasReportSubItem, index: number) => (
  ...
  toolbarClick = (args: ClickEventArgs) => {
    if (this.grid && args.item.id === 'Grid_excelexport') { 
      let excelExportProperties = {
        ...
        columns: [{field: "name", headerText:'Name', width: 150 }, {field: "bushcraft", headerText:'Bushcraft', width: 150 }, {field: "bushwalking", headerText:'Bushwalking', width: 150  }, {field: "camping", headerText:'Camping', width: 150 }, {field: "alpine", headerText:'Alpine', width: 150 }, {field: "cycling", headerText:'Cycling', width: 150 }, {field: "vertical", headerText:'Vertical', width: 150 }, {field: "aquatics", headerText:'Aquatics', width: 150 }, {field: "boating", headerText:'Boating', width: 150 }, {field: "paddling", headerText:'Paddling', width: 150} ] as Column[],
    ...
    if (this.grid && args.item.id === 'Grid_pdfexport') {
      const PdfExportProperties = {
        ...
        columns: [ {field: "name", headerText:'Name', width: 130 }, {field: "bushcraft", headerText:'Bushcraft', width: 95 }, {field: "bushwalking", headerText:'Bushwalking', width: 105  }, {field: "camping", headerText:'Camping', width: 95 }, {field: "alpine", headerText:'Alpine', width: 95 }, {field: "cycling", headerText:'Cycling', width: 95 }, {field: "vertical", headerText:'Vertical', width: 95 }, {field: "aquatics", headerText:'Aquatics', width: 95 }, {field: "boating", headerText:'Boating', width: 95 }, {field: "paddling", headerText:'Paddling', width: 95} ] as Column[],
  ...
  pdfQueryCellInfo = (args: PdfQueryCellInfoEventArgs) => {
    if (args.column && args.column.headerText != 'Name') {
      args.value = (args.data as OasReportItem)[args.column.field].map((stream: OasReportSubItem, index: number) => {
  ...
  render(): React.ReactNode {
    return (
      <GridComponent id="Grid" dataSource={this.props.items} allowResizing={true} toolbar={['Print', 'PdfExport', 'ExcelExport']} allowPdfExport={true} allowExcelExport={true} toolbarClick={this.toolbarClick} ref={(g: GridComponent | null) => this.grid = g} pdfQueryCellInfo={this.pdfQueryCellInfo} excelQueryCellInfo={this.excelQueryCellInfo} allowSorting={true}>
      <ColumnsDirective>
