...
export async function testReport() {
  ...
  $("#myTable").DataTable({
    data: dataSet,
    buttons: [{ extend: "create", editor } as never, { extend: "edit", editor } as never, { extend: "remove", editor } as never],
    columns: [{ title: "Name" }, { title: "Position" }, { title: "Office" }, { title: "Extn" }, { title: "Start" }, { title: "Salary" }],
    dom: "Bfrtip",
    select: true,
  });
  ...
  //create chart
  new Chart(document.getElementById("myChart") as HTMLCanvasElement, chartConfig);
}
